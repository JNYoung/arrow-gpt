...
> 新的总览和 debug 广告演示命令见 `docs/ad-platform-setup.md`。

## 后台创建状态

已在 AdMob 后台为 Arrow Again 创建两套 app：
...
AdMob 后台生成的值已填入 `platform-manifest.json`：
...
只做 SDK 冒烟测试时可以写入 Google sample app id：

```bash
npm run admob:sync:test
```

Android debug 包更推荐使用不污染 release 配置的演示命令：
